  ...
  <!-- Slide Show  -->
	<div id="bid2">  
		<center><table border="10">		
			...
			<?php
                           $itemid=$_SESSION['itemid'];
			   $conn = mysqli_connect("localhost","","",""); //open a connection to a mysql server
   ...
   if(array_key_exists('wmail',$_POST)){
              ...
              $ctime=$abid->viewTime();
               date_default_timezone_set('Asia/Kolkata');
                ...
                $close_timestamp =  strtotime($ctime);
                if($current_timestamp>$close_timestamp)
                 {
                   //echo 'EMAIL-'.$email=$abid->temp1;
                   
                   
		  // $subject = "Status Of Auction";
                   //$headers = 'Bidding Has Done Congratulation ';
		   //$message = "!!  Congratulations !! You are the winner of the bid ";

		  // if(mail($abid->temp1, $subject, $messsage, $headers)){
                     //   echo 'Mail Sent Done ';
                  // }else{
                   //     echo 'Cant Send mail';
                  // }
                   
                  $abid->sendConfirmationBuyer($abid->temp1);
                   } 
                else
                  {
